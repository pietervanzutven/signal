(function () {
    ...
    window.glob = async (pattern, options, callback) => {
        ...
        const queryOptions = new Windows.Storage.Search.QueryOptions();
        if (fileTypefilter !== '*') {
            queryOptions.fileTypeFilter = [fileTypefilter];
        }
        if (folderDepth === '**') {
            queryOptions.folderDepth = Windows.Storage.Search.FolderDepth.deep;
        }

        const folder = await Windows.Storage.ApplicationData.current.localFolder.getFolderAsync(parts.join('/'));
