function require(input) {
    if (typeof input === 'string') {
        ...
        if (input.includes('.')) {
            ...
            module = uri.absoluteUri.replace('/ts/','/typescript/');
        } else {
        ...
        if (require.modules[module]) {
            ...
            return window[input];
        } else if (window[input.replace(/-/g,'_').replace(/@/g,'')]) {
            return window[input.replace(/-/g,'_').replace(/@/g,'')];
        } else {
            const parts = module.replace('ms-appx://signal/','').replace('node_modules/','').replace('js/modules/','').split('/');
            let object = window[parts[0].replace('typescript','ts').replace(/-/g,'_').replace(/@/,'')];
            for (let i = 1; i < parts.length; i++) {
                object = object[parts[i].replace('typescript','ts').replace(/-/g,'_').replace(/@/,'')];
            }
            return object;
        }
    } else {
