function require(input) {
    if (typeof input === 'string') {
        ...
        if (require.modules[module]) {
            ...
            return window[input];
        } else if (window[input.replace(/-/g,'_')]) {
            return window[input.replace(/-/g,'_')];
        } else {
            const parts = module.replace('ms-appx://signal/','').split('/');
            let object = window[parts[0].replace('typescript','ts').replace(/-/g,'_')];
            for (let i = 1; i < parts.length; i++) {
                object = object[parts[i].replace('typescript','ts').replace(/-/g,'_')];
            }
            return object;
        }
    } else {
