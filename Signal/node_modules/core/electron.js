(function () {
  ...
  window.onload = () => {
    ...
    const element = document.getElementById('setupAsStandaloneLink');
    if (element) {
      element.addEventListener('click', () => {
          window.setupAsStandalone();
          return false;
      });
    }
  }
  ...
  window.electron = {
    app: {
      ...
      quit: () => { },
      isPackaged: !applicationModel.Package.current.isDevelopmentMode
    },
    ...
    BrowserWindow(options) {
      ...
      this.loadURL = src => {
        if (window.document.body && this.frame) {
          ...
          this.frame.src = src.replace('file://', '').replace('ms-appx://','ms-appx:///').replace(/-----BEGIN%20CERTIFICATE-----.*?-----END%20CERTIFICATE-----%0A/g,'');
        }
      }
      this.show = () => {
    ...
    ipc: {
      ...
      on: function (channel, listener) {
        window.electron.ipc.events[channel] = listener;
      },

      handle: function (channel, listener) {
        window.electron.ipc.events[channel] = function (...args) {
          listener(...args);
        }
      },

      once: function (channel, listener) {
        window.electron.ipc.events[channel] = function () {
          listener.apply(null, arguments);
          delete window.electron.ipc.events[channel];
        }
      },
      send: function () {
        ...
        const event = { 
            ...
            returnValue: null,
            sender: { 
                send: window.electron.ipc.send,
                isDestroyed: () => !window.electron.ipc.events.hasOwnProperty(channel)
            }
        };
        args[0] = event;
        if (window.electron.ipc.events[channel]) {
          window.electron.ipc.events[channel].apply(null, args);
        } else {
      ...
      sendSync: function () {
        return window.electron.ipc.send.apply(null, arguments);
      },
