...
(function () {
  ...
  const { computeHash } = window.types.conversation;
  ...
  async function _addAttachmentToMessage(message, attachment, { type, index }) {
    ...
    if (type === 'preview') {
      ...
      newPreview[index] = Object.assign({},
        preview[index],
        {
          image: attachment,
        }
    ...
    if (type === 'contact') {
      ...
      if (item && item.avatar && item.avatar.avatar) {
        ...
        newContact[index] = Object.assign({},
          contact[index],
          {
    ...
    if (type === 'quote') {
      ...
      const newAttachments = [...attachments];
      newAttachments[index] = Object.assign({},
        attachments[index],
        {
