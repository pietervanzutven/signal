...
const { copyIntoTempDirectory, deleteDraftFile, deleteTempFile, getAbsoluteAttachmentPath, getAbsoluteDraftPath, getAbsoluteTempPath, openFileInFolder, readAttachmentData, readDraftData, saveAttachmentToDisk, upgradeMessageSchema, writeNewDraftData, } = window.Signal.Migrations;
...
Whisper.TapToViewExpiredIncomingToast = Whisper.ToastView.extend({
    render_attributes() {
        return {
            toastMessage: window.i18n('Message--tap-to-view--incoming--expired-toast'),
        };
    },
});
Whisper.TapToViewExpiredOutgoingToast = Whisper.ToastView.extend({
    render_attributes() {
        return {
            toastMessage: window.i18n('Message--tap-to-view--outgoing--expired-toast'),
        };
    },
});
...
Whisper.ConversationView = Whisper.View.extend({
    ...
    initialize(options) {
        ...
        this.listenTo(this.model.messageCollection, 'download-new-version', this.downloadNewVersion);
        this.lazyUpdateVerified = _.debounce(this.model.updateVerified.bind(this.model), 1000 // one second
        );
        ...
        const attachmentListEl = $('<div class="module-composition-area__attachment-list"></div>');
    ...
    setupCompositionArea({ attachmentListEl }) {
      ...
      `)[0];
        ...
        this.compositionAreaView = new Whisper.ReactWrapperView({
            ...
            JSX: window.Signal.State.Roots.createCompositionArea(window.reduxStore, props),
        });
    ...
    setupTimeline() {
        ...
        const loadOlderMessages = async (oldestMessageId) => {
            ...
            try {
                ...
                messagesAdded(id, models.map(model => model.getReduxData()), isNewMessage, window.isActive());
            }
            catch (error) {
                setMessagesLoading(conversationId, true);
                throw error;
    ...
    async cleanModels(collection) {
        ...
        if (eliminated > 0) {
            window.log.warn(`cleanModels: Eliminated ${eliminated} messages without an id`);
        }
    ...
    async loadAndScroll(messageId, options) {
        ...
        try {
            ...
            messagesReset(conversationId, cleaned.map((model) => model.getReduxData()), metrics, scrollToMessageId);
        }
        catch (error) {
            setMessagesLoading(conversationId, false);
            throw error;
        ...
        try {
            ...
            if (newestMessageId) {
                ...
                if (!newestInMemoryMessage) {
                    window.log.warn(`loadNewestMessages: did not find message ${newestMessageId}`);
                }
            ...
            messagesReset(conversationId, cleaned.map((model) => model.getReduxData()), metrics, scrollToMessageId, unboundedFetch);
        }
        catch (error) {
            setMessagesLoading(conversationId, false);
            throw error;
    ...
    unload(reason) {
        window.log.info('unloading conversation', this.model.idForLogging(), 'due to:', reason);
    ...
    async onCloseAttachment(attachment) {
        const draftAttachments = this.model.get('draftAttachments') || [];
        this.model.set({
            draftAttachments: _.reject(draftAttachments, item => item.path === attachment.path),
            draftChanged: true,
        });
        this.updateAttachmentsView();
        await this.saveModel();
    ...
    async clearAttachments() {
        ...
        await Promise.all([
            this.saveModel(),
            Promise.all(draftAttachments.map((attachment) => this.deleteDraftAttachment(attachment))),
        ]);
    },
    hasFiles() {
        const draftAttachments = this.model.get('draftAttachments') || [];
    ...
    async getFiles() {
        ...
        const draftAttachments = this.model.get('draftAttachments') || [];
        const files = _.compact(await Promise.all(draftAttachments.map((attachment) => this.getFile(attachment))));
        return files;
    },
    async getFile(attachment) {
        ...
        if (data.byteLength !== attachment.size) {
            window.log.error(`Attachment size from disk ${data.byteLength} did not match attachment size ${attachment.size}`);
            return null;
        }
        return Object.assign(Object.assign({}, _.pick(attachment, [
            'contentType',
            'fileName',
            'size',
            'caption',
    ...
    async writeDraftAttachment(attachment) {
        ...
        if (toWrite.data) {
            ...
            toWrite = Object.assign(Object.assign({}, _.omit(toWrite, ['data'])), { path });
        }
        if (toWrite.screenshotData) {
            const screenshotPath = await writeNewDraftData(toWrite.screenshotData);
            toWrite = Object.assign(Object.assign({}, _.omit(toWrite, ['screenshotData'])), { screenshotPath });
        }
        return toWrite;
    },
    async maybeAddAttachment(file) {
        ...
        const haveNonImage = window._.any(draftAttachments, (attachment) => !MIME.isImage(attachment.contentType));
        ...
        try {
            ...
            else if (window.Signal.Util.GoogleChrome.isVideoTypeSupported(file.type)) {
            ...
            window.log.error(`Was unable to generate thumbnail for file type ${file.type}`, e && e.stack ? e.stack : e);
            const data = await this.arrayBufferFromFile(file);
            attachment = {
                data,
                size: data.byteLength,
    ...
    isSizeOkay(attachment) {
        let limitKb = 1000000;
        const type = attachment.contentType === 'image/gif'
            ? 'gif'
    ...
    async handleVideoAttachment(file) {
        ...
        try {
            ...
            const screenshotData = await VisualAttachment.blobToArrayBuffer(screenshotBlob);
            const data = await this.arrayBufferFromFile(file);
            return {
                fileName: file.name,
    ...
    autoScale(attachment) {
        ...
        return new Promise((resolve, reject) => {
            ...
            img.onload = () => {
                ...
                do {
                    i -= 1;
                    blob = window.dataURLToBlobSync(canvas.toDataURL(targetContentType, quality));
                ...
                resolve(Object.assign(Object.assign({}, attachment), { fileName: this.fixExtension(fileName, targetContentType), contentType: targetContentType, file: blob }));
            };
            img.src = url;
        });
    },
    ...
    async showAllMedia() {
        ...
        const getProps = async () => {
            ...
            const media = window._.flatten(rawMedia.map(message => {
            ...
            const onItemClick = async ({ message, attachment, type }) => {
                switch (type) {
                    ...
                    case 'media': {
                        const selectedIndex = media.findIndex(mediaMessage => mediaMessage.attachment.path === attachment.path);
    ...
    downloadAttachmentWrapper(messageId) {
        const message = this.model.messageCollection.get(messageId);
        if (!message) {
            throw new Error(`downloadAttachmentWrapper: Did not find message for id ${messageId}`);
        }
    ...
    async displayTapToViewMessage(messageId) {
        const message = this.model.messageCollection.get(messageId);
        if (!message) {
            throw new Error(`displayTapToViewMessage: Did not find message for id ${messageId}`);
        }
    ...
    showLightbox({ attachment, messageId }) {
        ...
        const selectedIndex = window._.findIndex(media, (item) => attachment.path === item.path);
    ...
    showMessageDetail(messageId) {
        const message = this.model.messageCollection.get(messageId);
        if (!message) {
            throw new Error(`showMessageDetail: Did not find message for id ${messageId}`);
        }
    ...
    setMuteNotifications(ms) {
        ...
        if (muteExpiresAt) {
            ...
            window.Signal.Services.onTimeout(muteExpiresAt, () => {
    ...
    async destroyMessages() {
        try {
            ...
            try {
                ...
                window.log.error('destroyMessages: Failed to successfully delete conversation', error && error.stack ? error.stack : error);
            }
        }
        catch (error) {
    ...
    async isCallSafe() {
        ...
        if (contacts && contacts.length) {
            ...
            if (!callAnyway) {
                window.log.info('Safety number change dialog not accepted, new call not allowed.');
                return false;
            }
        }
        return true;
    ...
    async sendStickerMessage(options = {}) {
        try {
            ...
            window.log.error('clickSend error:', error && error.stack ? error.stack : error);
        }
    },
    ...
    async getStickerPackPreview(url) {
        ...
        try {
            ...
            return {
                title,
                url,
                image: Object.assign(Object.assign({}, sticker), { data, size: data.byteLength, contentType: 'image/webp' }),
            };
        }
        catch (error) {
            window.log.error('getStickerPackPreview error:', error && error.stack ? error.stack : error);
            return null;
        }
        finally {
            if (id) {
    ...
    async addLinkPreview(url) {
        ...
        try {
            ...
            window.log.error('Problem loading link preview, disabling.', error && error.stack ? error.stack : error);
    ...
    getLinkPreview() {
        ...
        return this.preview.map((item) => {
            if (item.image) {
                ...
                return Object.assign(Object.assign({}, item), { image: _.omit(item.image, 'url') });
            }
            return item;
        });
    },
