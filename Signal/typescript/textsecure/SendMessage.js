(function () {
    ...
    class Message {
        ...
        toProto() {
            ...
            if (this.sticker) {
                ...
                proto.sticker.packId = Crypto_2.hexToArrayBuffer(this.sticker.packId);
                proto.sticker.packKey = Crypto_2.base64ToArrayBuffer(this.sticker.packKey);
    ...
    class MessageSender {
        ...
        getPaddedAttachment(data) {
            ...
            const padding = Crypto_2.getZeroes(paddedSize - size);
            return Crypto_2.concatenateBytes(data, padding);
        }
        async makeAttachmentPointer(attachment) {
            ...
            if (!(data instanceof ArrayBuffer) && !ArrayBuffer.isView(data)) {
                throw new Error(`makeAttachmentPointer: data was a '${typeof data}' instead of ArrayBuffer/ArrayBufferView`);
            }
        ...
        async queueJobForIdentifier(identifier, runJob) {
            ...
            const taskWithTimeout = TaskWithTimeout_1.default(runJob, `queueJobForIdentifier ${identifier} ${id}`);
            return queue.add(taskWithTimeout);
        }
        async uploadAttachments(message) {
            return Promise.all(message.attachments.map(this.makeAttachmentPointer.bind(this)))
        ...
        async uploadLinkPreviews(message) {
            try {
                const preview = await Promise.all((message.preview || []).map(async (item) => (Object.assign(Object.assign({}, item), { image: await this.makeAttachmentPointer(item.image) }))));
                // eslint-disable-next-line no-param-reassign
        ...
        async sendMessage(attrs, options) {
            ...
            return Promise.all([
                ...
                this.sendMessageProto(message.timestamp, message.recipients || [], message.toProto(), (res) => {
        ...
        sendMessageProto(timestamp, recipients, messageProto, callback, silent, options) {
        ...
        async sendMessageProtoAndWait(timestamp, identifiers, messageProto, silent, options) {
        ...
        async sendIndividualProto(identifier, proto, timestamp, silent, options) {
            return new Promise((resolve, reject) => {
                const callback = (res) => {
                ...
                this.sendMessageProto(timestamp, [identifier], proto, callback, silent, options);
            });
        }
        createSyncMessage() {
        ...
        async sendSyncMessage(encodedDataMessage, timestamp, destination, destinationUuid, expirationStartTimestamp, sentTo = [], unidentifiedDeliveries = [], isUpdate = false, options) {
            ...
            const dataMessage = window.textsecure.protobuf.DataMessage.decode(encodedDataMessage);
            ...
            const unidentifiedLookup = unidentifiedDeliveries.reduce((accumulator, item) => {
                // eslint-disable-next-line no-param-reassign
        ...
        async sendRequestBlockSyncMessage(options) {
            ...
            if (myDevice !== 1 && myDevice !== '1') {
                ...
                contentMessage.syncMessage = syncMessage;
                const silent = true;
                return this.sendIndividualProto(myUuid || myNumber, contentMessage, Date.now(), silent, options);
            }
            return Promise.resolve();
        }
        ...
        async sendTypingMessage(options, sendOptions = {}) {
            ...
            const silent = true;
            const online = true;
            return this.sendMessageProtoAndWait(finalTimestamp, recipients, contentMessage, silent, Object.assign(Object.assign({}, sendOptions), { online }));
        }
        async sendProfileKeyUpdate(profileKey, recipients, sendOptions, groupId) {
            return this.sendMessage(Object.assign({ recipients, timestamp: Date.now(), profileKey, flags: window.textsecure.protobuf.DataMessage.Flags.PROFILE_KEY_UPDATE }, (groupId
                ? {
                    group: {
                        id: groupId,
        ...
        async sendDeliveryReceipt(recipientE164, recipientUuid, timestamps, options) {
            ...
            const myUuid = window.textsecure.storage.user.getUuid();
            const myDevice = window.textsecure.storage.user.getDeviceId();
            if ((myNumber === recipientE164 || myUuid === recipientUuid) &&
            ...
            const silent = true;
            return this.sendIndividualProto(recipientUuid || recipientE164, contentMessage, Date.now(), silent, options);
        }
        async sendReadReceipts(senderE164, senderUuid, timestamps, options) {
            const receiptMessage = new window.textsecure.protobuf.ReceiptMessage();
            ...
            contentMessage.receiptMessage = receiptMessage;
            const silent = true;
            return this.sendIndividualProto(senderUuid || senderE164, contentMessage, Date.now(), silent, options);
        }
        ...
        async syncMessageRequestResponse(responseArgs, sendOptions) {
            const myNumber = window.textsecure.storage.user.getNumber();
            const myUuid = window.textsecure.storage.user.getUuid();
            const myDevice = window.textsecure.storage.user.getDeviceId();
            if (myDevice === 1 || myDevice === '1') {
            ...
            contentMessage.syncMessage = syncMessage;
            const silent = true;
            return this.sendIndividualProto(myUuid || myNumber, contentMessage, Date.now(), silent, sendOptions);
        }
        async sendStickerPackSync(operations, options) {
            ...
            const packOperations = operations.map(item => {
                ...
                operation.packId = Crypto_2.hexToArrayBuffer(packId);
                operation.packKey = Crypto_2.base64ToArrayBuffer(packKey);
        ...
        async syncVerification(destinationE164, destinationUuid, state, identityKey, options) {
            ...
            const silent = false;
            const promise = this.sendIndividualProto(destinationUuid || destinationE164, contentMessage, now, silent, options);
            return promise.then(async () => {
                ...
                return this.sendIndividualProto(myUuid || myNumber, secondMessage, now, innerSilent, options);
            });
        }
        async sendGroupProto(providedIdentifiers, proto, timestamp = Date.now(), options = {}) {
