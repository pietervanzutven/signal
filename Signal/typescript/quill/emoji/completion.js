...
class EmojiCompletion {
    ...
    onTextChange() {
        ...
        const leftTokenTextMatch = /(?<=^|\s):([-+0-9a-z_]*)(:?)$/.exec(leftLeafText);
    ...
    render() {
        ...
        const element = react_dom_1.createPortal(react_1.default.createElement(react_popper_1.Popper, { placement: "top", modifiers: {
                width: {
                    enabled: true,
                    fn: oldData => {
                        const data = oldData;
            ...
            } }, ({ ref, style }) => (react_1.default.createElement("div", { ref: ref, className: "module-composition-input__emoji-suggestions", style: style, role: "listbox", "aria-expanded": true, "aria-activedescendant": `emoji-result--${emojiResults.length
                ? emojiResults[emojiResultsIndex].short_name
                : ''}`, tabIndex: 0 }, emojiResults.map((emoji, index) => (react_1.default.createElement("button", { type: "button", key: emoji.short_name, id: `emoji-result--${emoji.short_name}`, role: "option button", "aria-selected": emojiResultsIndex === index, onClick: () => {
                this.index = index;
                this.completeEmoji();
