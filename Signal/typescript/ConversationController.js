require(exports => {
    ...
    function start() {
        ...
        const inboxCollection = new (window.Backbone.Collection.extend({
            ...
            updateUnreadCount() {
                ...
                const canCount = (m) => !m.isMuted() || canCountMutedConversations;
                ...
                const newUnreadCount = lodash_1.reduce(this.map((m) => canCount(m) ? getUnreadCount(m) : 0), (item, memo) => (item || 0) + memo, 0);
    ...
    class ConversationController {
        ...
        getOrCreate(identifier, type, additionalInitialProps = {}) {
            ...
            if (type === 'group') {
                conversation = this._conversations.add(Object.assign({ id, uuid: null, e164: null, groupId: identifier, type, version: 2 }, additionalInitialProps));
            }
            else if (window.isValidGuid(identifier)) {
                conversation = this._conversations.add(Object.assign({ id, uuid: identifier, e164: null, groupId: null, type, version: 2 }, additionalInitialProps));
            }
            else {
                conversation = this._conversations.add(Object.assign({ id, uuid: null, e164: identifier, groupId: null, type, version: 2 }, additionalInitialProps));
            }
            const create = async () => {
                if (!conversation.isValid()) {
                    const validationError = conversation.validationError || {};
                    window.log.error('Contact is not valid. Not saving, but adding to collection:', conversation.idForLogging(), validationError.stack);
                    return conversation;
                }
                try {
                    ...
                    window.log.error('Conversation save failed! ', identifier, type, 'Error:', error && error.stack ? error.stack : error);
                    throw error;
                }
                return conversation;
            };
     ...
     * highTrust = uuid/e164 pairing came from CDS, the server, or your own device
        ...
        ensureContactIds({ e164, uuid, highTrust, }) {
            ...
            if (!convoE164 && !convoUuid) {
                window.log.info('ensureContactIds: Creating new contact, no matches found');
            ...
            if (convoE164 && !convoUuid) {
                ...
                window.log.info('ensureContactIds: e164 already had UUID, creating a new contact');
                // If existing e164 match already has UUID, create a new contact...
                const newConvo = this.getOrCreate(normalizedUuid, 'private');
                if (highTrust) {
                    window.log.info('ensureContactIds: Moving e164 from old contact to new');
            ...
            if (highTrust) {
                ...
                if (convoE164.get('uuid') && convoE164.get('uuid') !== normalizedUuid) {
                    window.log.info('ensureContactIds: e164 match had different UUID than incoming pair, removing its e164.');
                    // Remove the e164 from the old contact...
                    convoE164.set({ e164: undefined });
                    updateConversation(convoE164.attributes);
                    // ...and add it to the new one.
                ...
                this.combineContacts(convoUuid, convoE164)
                    ...
                    .catch(error => {
                        ...
                        window.log.warn(`ensureContactIds error combining contacts: ${errorText}`);
                    });
            }
            return convoUuid.get('id');
        }
            ...
            for (let i = 0; i < max; i += 1) {
                ...
                if (uuid) {
                    ...
                    if (!existing) {
                        ...
                        window.log.warn(`checkForConflicts: Found conflict with uuid ${uuid}`);
                ...
                if (e164) {
                    ...
                    if (!existing) {
                        ...
                        if (conversation.get('uuid') &&
                            ...
                            window.log.warn(`checkForConflicts: Found two matches on e164 ${e164} with different truthy UUIDs. Dropping e164 on older.`);
        ...
        async combineContacts(current, obsolete) {
            ...
            await Promise.all(deviceIds.map(async (deviceId) => {
                await window.textsecure.storage.protocol.removeSession(`${obsoleteId}.${deviceId}`);
            }));
            window.log.warn('combineContacts: Delete all identity information tied to old conversationId');
            await window.textsecure.storage.protocol.removeIdentityKey(obsoleteId);
            window.log.warn('combineContacts: Ensure that all V1 groups have new conversationId instead of old');
            const groups = await this.getAllGroupsInvolvingId(obsoleteId);
            groups.forEach(group => {
                const members = group.get('members');
                const withoutObsolete = lodash_1.without(members, obsoleteId);
                const currentAdded = lodash_1.uniq([...withoutObsolete, currentId]);
                group.set({
            ...
            window.log.warn('combineContacts: Delete the obsolete conversation from the database');
        ...
        getPinnedConversationIds() {
            let pinnedConversationIds = window.storage.get('pinnedConversationIds');
            ...
            if (pinnedConversationIds === undefined) {
                window.log.info('getPinnedConversationIds: no pinned conversations in storage');
                ...
                window.log.info(`getPinnedConversationIds: falling back to ${modelPinnedConversationIds.length} pinned models`);
