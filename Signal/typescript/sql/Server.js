require(exports => {
    ...
    function promisify(rawInstance) {
        return {
            close: pify_1.default(rawInstance.close.bind(rawInstance)),
    ...
    async function migrateSchemaVersion(instance) {
        ...
        console.log(`migrateSchemaVersion: Migrating from schema_version ${schemaVersion} to user_version ${newUserVersion}`);
    ...
    async function updateToSchemaVersion1(currentVersion, instance) {
        ...
        try {
            await instance.run(`CREATE TABLE messages(
    ...
    async function updateToSchemaVersion2(currentVersion, instance) {
        ...
        await instance.run('BEGIN TRANSACTION;');
        try {
            await instance.run(`ALTER TABLE messages
     ...
     ADD COLUMN type STRING;`);
      ...
      expires_at
            ...
            await instance.run(`UPDATE messages SET
    ...
    async function updateToSchemaVersion4(currentVersion, instance) {
        ...
        await instance.run('BEGIN TRANSACTION;');
        try {
            await instance.run(`CREATE TABLE conversations(
      id STRING PRIMARY KEY ASC,
      json TEXT,
    ...
    async function updateToSchemaVersion6(currentVersion, instance) {
        ...
        try {
            ...
            await instance.run(`CREATE TABLE sessions(
      id STRING PRIMARY KEY ASC,
      number STRING,
      json TEXT
    );`);
    ...
    number
            ...
            await instance.run(`CREATE TABLE groups(
      id STRING PRIMARY KEY ASC,
      json TEXT
    );`);
            await instance.run(`CREATE TABLE identityKeys(
      id STRING PRIMARY KEY ASC,
      json TEXT
    );`);
            await instance.run(`CREATE TABLE items(
      id STRING PRIMARY KEY ASC,
      json TEXT
    );`);
            // key-value, ids are integers
            await instance.run(`CREATE TABLE preKeys(
      id INTEGER PRIMARY KEY ASC,
      json TEXT
    );`);
            await instance.run(`CREATE TABLE signedPreKeys(
      id INTEGER PRIMARY KEY ASC,
      json TEXT
    );`);
  ...
  `);
    ...
    async function updateToSchemaVersion13(currentVersion, instance) {
        ...
        await instance.run('BEGIN TRANSACTION;');
        try {
            await instance.run('ALTER TABLE sticker_packs ADD COLUMN attemptedStatus STRING;');
    ...
    async function updateToSchemaVersion17(currentVersion, instance) {
        ...
        ADD COLUMN isViewOnce INTEGER;`);
                ...
                console.log('updateToSchemaVersion17: Message table already had isViewOnce column');
            }
            try {
                await instance.run('DROP INDEX messages_view_once;');
            }
            catch (error) {
                console.log('updateToSchemaVersion17: Index messages_view_once did not already exist');
            }
    ...
    async function updateToSchemaVersion18(currentVersion, instance) {
        ...
        try {
            ...
            await instance.run("INSERT INTO messages_fts(messages_fts) VALUES('rebuild');");
            await instance.run(`
      INSERT INTO messages_fts(id, body)
    ...
    async function updateToSchemaVersion19(currentVersion, instance) {
        ...
        await instance.run('BEGIN TRANSACTION;');
        await instance.run(`ALTER TABLE conversations
    ...
    async function updateToSchemaVersion20(currentVersion, instance) {
        ...
        try {
            ...
            // Drop triggers
            const triggers = await instance.all('SELECT * FROM sqlite_master WHERE type = "trigger" AND tbl_name = "messages"');
            ...
            await instance.run('ALTER TABLE sessions RENAME COLUMN number TO conversationId;');
            await instance.run('CREATE INDEX conversations_e164 ON conversations(e164);');
            await instance.run('CREATE INDEX conversations_uuid ON conversations(uuid);');
            await instance.run('CREATE INDEX conversations_groupId ON conversations(groupId);');
            await instance.run('CREATE INDEX messages_sourceUuid on messages(sourceUuid);');
            // Migrate existing IDs
            await instance.run("UPDATE conversations SET e164 = '+' || id WHERE type = 'private';");
            await instance.run("UPDATE conversations SET groupId = id WHERE type = 'group';");
            // Drop invalid groups and any associated messages
            const maybeInvalidGroups = await instance.all("SELECT * FROM conversations WHERE type = 'group' AND members IS NULL;");
            ...
            for (const row of allConversations) {
                ...
                await instance.run('UPDATE conversations SET id = $newId, json = JSON_PATCH(json, $patch) WHERE id = $oldId', {
                    $newId: newId,
                    $oldId: oldId,
                    $patch: patch,
                });
                const messagePatch = JSON.stringify({ conversationId: newId });
                await instance.run('UPDATE messages SET conversationId = $newId, json = JSON_PATCH(json, $patch) WHERE conversationId = $oldId', { $newId: newId, $oldId: oldId, $patch: messagePatch });
            }
            ...
            migrationJobQueue.addAll(groupConverations.map(groupRow => async () => {
                ...
                await instance.run('UPDATE conversations SET members = $newMembersValue, json = $newJsonValue WHERE id = $id', {
                    $id: groupRow.id,
    ...
    async function updateToSchemaVersion21(currentVersion, instance) {
    ...
    async function createOrUpdateSessions(array) {
        ...
        try {
            await Promise.all([
                ...lodash_1.map(array, async (item) => createOrUpdateSession(item)),
            ]);
    ...
    async function getSessionsById(conversationId) {
        const db = getInstance();
        const rows = await db.all('SELECT * FROM sessions WHERE conversationId = $conversationId;', {
            $conversationId: conversationId,
        });
        return lodash_1.map(rows, row => jsonToObject(row.json));
    }
    ...
    async function createOrUpdate(table, data) {
        ...
        await db.run(`INSERT OR REPLACE INTO ${table} (
      id,
      json
    ) values (
      $id,
    ...
    async function bulkAdd(table, array) {
        ...
        try {
            await Promise.all([
                ...lodash_1.map(array, async (data) => createOrUpdate(table, data)),
            ]);
            await db.run('COMMIT TRANSACTION;');
        }
        catch (error) {
    ...
    $profileFullName
            ...
            $profileFullName: combineNames_1.combineNames(profileName, profileFamilyName),
        });
    }
    async function saveConversations(arrayOfConversations) {
        ...
        try {
            await Promise.all([
                ...lodash_1.map(arrayOfConversations, async (conversation) => saveConversation(conversation)),
            ]);
            await db.run('COMMIT TRANSACTION;');
        }
        catch (error) {
    ...
    async function updateConversations(array) {
        ...
        await db.run('BEGIN TRANSACTION;');
        try {
            await Promise.all([...lodash_1.map(array, async (item) => updateConversation(item))]);
            await db.run('COMMIT TRANSACTION;');
        }
        catch (error) {
            await db.run('ROLLBACK;');
    ...
    async function removeConversation(id) {
        ...
        // Our node interface doesn't seem to allow you to replace one single ? with an array
        await db.run(`DELETE FROM conversations WHERE id IN ( ${id
            .map(() => '?')
            .join(', ')} );`, id);
    }
    ...
    async function getAllConversationIds() {
        ...
        return lodash_1.map(rows, row => row.id);
    }
    ...
    async function getMessageCount(conversationId) {
        ...
        const row = conversationId
            ? await db.get('SELECT count(*) from messages WHERE conversationId = $conversationId;', { $conversationId: conversationId })
    ...
    $type,
    $unread
  );`, Object.assign(Object.assign({}, payload), { $id: toCreate.id, $json: objectToJSON(toCreate) }));
        return toCreate.id;
    }
    async function saveMessages(arrayOfMessages, { forceSave } = {}) {
        ...
        try {
            await Promise.all([
                ...lodash_1.map(arrayOfMessages, async (message) => saveMessage(message, { forceSave })),
            ]);
            await db.run('COMMIT TRANSACTION;');
        }
        catch (error) {
    ...
    async function getTotalUnreadForConversation(conversationId) {
        const db = getInstance();
        const row = await db.get(`SELECT count(id) from messages WHERE
       conversationId = $conversationId AND
       unread = 1;
    `, {
            $conversationId: conversationId,
  ...
  `);
    ...
    async function getTapToViewMessagesNeedingErase() {
        ...
        const rows = await db.all(`SELECT json FROM messages
    WHERE
      isViewOnce = 1
      AND (isErased IS NULL OR isErased != 1)
    ...
    async function saveUnprocessed(data, { forceSave } = {}) {
        ...
        if (forceSave) {
            await db.run(`INSERT INTO unprocessed (
        id,
        timestamp,
        version,
        attempts,
        ...
        await db.run(`UPDATE unprocessed SET
      timestamp = $timestamp,
      version = $version,
      attempts = $attempts,
      envelope = $envelope
    ...
    async function saveUnprocesseds(arrayOfUnprocessed, { forceSave } = {}) {
        ...
        try {
            await Promise.all([
                ...lodash_1.map(arrayOfUnprocessed, async (unprocessed) => saveUnprocessed(unprocessed, { forceSave })),
            ]);
            await db.run('COMMIT TRANSACTION;');
        }
        catch (error) {
    ...
    async function updateUnprocessedsWithData(arrayOfUnprocessed) {
        ...
        try {
            await Promise.all([
                ...lodash_1.map(arrayOfUnprocessed, async ({ id, data }) => updateUnprocessedWithData(id, data)),
            ]);
            await db.run('COMMIT TRANSACTION;');
        }
        catch (error) {
    ...
    async function getNextAttachmentDownloadJobs(limit, options = {}) {
        ...
        const rows = await db.all(`SELECT json FROM attachment_downloads
    ...
    async function saveAttachmentDownloadJob(job) {
        ...
        await db.run(`INSERT OR REPLACE INTO attachment_downloads (
      id,
      pending,
      timestamp,
      json
    ...
    async function createOrUpdateStickerPack(pack) {
        ...
        if (rows && rows.length) {
            await db.run(`UPDATE sticker_packs SET
        attemptedStatus = $attemptedStatus,
        author = $author,
        coverStickerId = $coverStickerId,
        createdAt = $createdAt,
      ...
      WHERE id = $id;`, payload);
        ...
        await db.run(`INSERT INTO sticker_packs (
      attemptedStatus,
      author,
      coverStickerId,
      createdAt,
    ...
    async function updateStickerPackStatus(id, status, options) {
        ...
        await db.run(`UPDATE sticker_packs
    ...
    async function createOrUpdateSticker(sticker) {
        ...
        await db.run(`INSERT OR REPLACE INTO stickers (
      emoji,
      height,
      id,
      isCoverOnly,
    ...
    async function addStickerPackReference(messageId, packId) {
        ...
        await db.run(`INSERT OR REPLACE INTO sticker_references (
      messageId,
      packId
    ) values (
      $messageId,
    ...
    async function deleteStickerPackReference(messageId, packId) {
        ...
        try {
            ...
            await db.run(`DELETE FROM sticker_references
      WHERE messageId = $messageId AND packId = $packId;`, {
                $messageId: messageId,
                $packId: packId,
            });
            const countRow = await db.get(`SELECT count(*) FROM sticker_references
      WHERE packId = $packId;`, { $packId: packId });
            if (!countRow) {
            ...
            const packRow = await db.get(`SELECT status FROM sticker_packs
      WHERE id = $packId;`, { $packId: packId });
            if (!packRow) {
            ...
            const stickerPathRows = await db.all(`SELECT path FROM stickers
      WHERE packId = $packId;`, {
                $packId: packId,
            });
            await db.run(`DELETE FROM sticker_packs
      WHERE id = $packId;`, { $packId: packId });
            await db.run('COMMIT TRANSACTION;');
    ...
    async function getAllStickerPacks() {
        const db = getInstance();
        const rows = await db.all(`SELECT * FROM sticker_packs
    ORDER BY installedAt DESC, createdAt DESC`);
        return rows || [];
    }
    async function getAllStickers() {
        const db = getInstance();
        const rows = await db.all(`SELECT * FROM stickers
    ...
    async function getRecentStickers({ limit } = {}) {
        const db = getInstance();
        // Note: we avoid 'IS NOT NULL' here because it does seem to bypass our index
        const rows = await db.all(`SELECT stickers.* FROM stickers
    ...
    async function updateEmojiUsage(shortName, timeUsed = Date.now()) {
        ...
        await db.run('BEGIN TRANSACTION;');
        try {
            const rows = await db.get('SELECT * FROM emojis WHERE shortName = $shortName;', {
                $shortName: shortName,
            });
            if (rows) {
                await db.run('UPDATE emojis SET lastUsage = $timeUsed WHERE shortName = $shortName;', { $shortName: shortName, $timeUsed: timeUsed });
            }
            else {
                await db.run('INSERT INTO emojis(shortName, lastUsage) VALUES ($shortName, $timeUsed);', { $shortName: shortName, $timeUsed: timeUsed });
            }
            await db.run('COMMIT TRANSACTION;');
        }
        catch (error) {
    ...
    async function getRecentEmojis(limit = 32) {
        const db = getInstance();
        const rows = await db.all('SELECT * FROM emojis ORDER BY lastUsage DESC LIMIT $limit;', {
            $limit: limit,
        });
        return rows || [];
    }
    ...
    async function removeKnownAttachments(allAttachments) {
       ...
       LIMIT $chunkSize;`, {
            ...
            const messages = lodash_1.map(rows, row => jsonToObject(row.json));
        ...
        console.log(`removeKnownAttachments: About to iterate through ${conversationTotal} conversations`);
        while (!complete) {
            const rows = await db.all(`SELECT json FROM conversations
       WHERE id > $id
       ORDER BY id ASC
       LIMIT $chunkSize;`, {
                $id: id,
    ...
    async function removeKnownStickers(allStickers) {
        ...
        let rowid = 0;
        while (!complete) {
            const rows = await db.all(`SELECT rowid, path FROM stickers
       WHERE rowid > $rowid
       ORDER BY rowid ASC
       LIMIT $chunkSize;`, {
                $rowid: rowid,
