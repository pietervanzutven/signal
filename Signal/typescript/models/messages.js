require(exports => {
    ...
    const { copyStickerToAttachments, deletePackReference, savePackMetadata, getStickerPackStatus, } = window.Signal.Stickers;
    ...
    window.isSignalAccountCheckComplete = number => window.AccountCache[number] !== undefined;
    ...
    class MessageModel extends window.Backbone.Model {
        ...
        getPropsForMessageDetail() {
            ...
            const unidentifiedLookup = (this.get('unidentifiedDeliveries') || []).reduce((accumulator, identifier) => {
                accumulator[window.ConversationController.getConversationId(identifier)] = true;
                return accumulator;
            }, Object.create(null));
            ...
            const conversationIds = this.isIncoming()
                ? [this.getContactId()]
                : _.union((this.get('sent_to') || []).map((id) => window.ConversationController.getConversationId(id)), (this.get('recipients') || this.getConversation().getRecipients()).map((id) => window.ConversationController.getConversationId(id)));
            ...
            const sortedContacts = _.sortBy(finalContacts, contact => `${contact.errors ? '0' : '1'}${contact.title}`);
            return {
                sentAt: this.get('sent_at'),
        ...
        isUnsupportedMessage() {
            ...
            return (_.isNumber(versionAtReceive) &&
        ...
        getPropsForGroupNotification() {
            ...
            if (groupUpdate.joined) {
                changes.push({
                    type: 'add',
                    contacts: _.map(Array.isArray(groupUpdate.joined)
                        ? groupUpdate.joined
                        : [groupUpdate.joined], identifier => this.findAndFormatContact(identifier)),
                });
            }
            if (groupUpdate.left === 'You') {
                ...
                changes.push({
                    type: 'remove',
                    contacts: _.map(Array.isArray(groupUpdate.left)
                        ? groupUpdate.left
                        : [groupUpdate.left], identifier => this.findAndFormatContact(identifier)),
                });
            }
            if (groupUpdate.name) {
                changes.push({
        ...
        getPropsForMessage() {
            ...
            const selectedReaction = ((this.get('reactions') || []).find(re => re.fromId === window.ConversationController.getOurConversationId()) || {}).emoji;
            return {
                ...
                isTapToViewError: isTapToView && this.isIncoming() && this.get('isTapToViewInvalid'),
        ...
        getPropsForPreview() {
            ...
            return previews.map(preview => (Object.assign(Object.assign({}, preview), { isStickerPack: window.Signal.LinkPreviews.isStickerPack(preview.url), domain: window.Signal.LinkPreviews.getDomain(preview.url), image: preview.image ? this.getPropsForAttachment(preview.image) : null })));
        }
        getPropsForQuote() {
            ...
            const regionCode = window.storage.get('regionCode');
            const { author, authorUuid, bodyRanges, id: sentAt, referencedMessageNotFound, text, } = quote;
            const contact = (author || authorUuid) &&
                window.ConversationController.get(window.ConversationController.ensureContactIds({
                    e164: author,
                    uuid: authorUuid,
                }));
        ...
        processQuoteAttachment(attachment) {
            ...
            const thumbnailWithObjectUrl = !path && !objectUrl
                ? null
                : Object.assign(Object.assign({}, (attachment.thumbnail || {})), { objectUrl: path || objectUrl });
            return Object.assign(Object.assign({}, attachment), { isVoiceMessage: window.Signal.Types.Attachment.isVoiceMessage(attachment), thumbnail: thumbnailWithObjectUrl });
        }
        getNotificationData() {
            if (this.isUnsupportedMessage()) {
                return {
            ...
            if (this.isGroupV2Change()) {
                ...
                const lines = window.Signal.GroupChange.renderChange(change, {
                    ...
                    renderString: (key, _i18n, placeholders) => window.i18n(key, placeholders),
                    RoleEnum: protobuf.Member.Role,
                });
            ...
            if (this.isGroupUpdate()) {
                ...
                if (groupUpdate.joined && groupUpdate.joined.length) {
                    ...
                    const joinedWithoutMe = joinedContacts.filter(contact => !contact.isMe());
            ...
            if (stickerData) {
                const sticker = window.Signal.Stickers.getSticker(stickerData.packId, stickerData.stickerId);
                const { emoji } = sticker || {};
                if (!emoji) {
        ...
        isValidTapToView() {
            ...
            if (!window.Signal.Util.GoogleChrome.isImageTypeSupported(firstAttachment.contentType) &&
                !window.Signal.Util.GoogleChrome.isVideoTypeSupported(firstAttachment.contentType)) {
                return false;
            }
            const quote = this.get('quote');
            const sticker = this.get('sticker');
        ...
        async markViewed(options) {
            ...
            if (!this.isValidTapToView()) {
                window.log.warn(`markViewed: Message ${this.idForLogging()} is not a valid tap to view message!`);
                return;
            }
            if (this.isErased()) {
                window.log.warn(`markViewed: Message ${this.idForLogging()} is already erased!`);
                return;
            }
            if (this.get('unread')) {
                await this.markRead();
            ...
            if (!fromSync) {
                ...
                const { wrap, sendOptions, } = window.ConversationController.prepareForSend(ourNumber || ourUuid, {
                    syncMessage: true,
                });
                await wrap(window.textsecure.messaging.syncViewOnceOpen(sender, senderUuid, timestamp, sendOptions));
            }
        }
        isErased() {
        ...
        async retrySend() {
            ...
            const intendedRecipients = (this.get('recipients') || [])
                .map(identifier => window.ConversationController.getConversationId(identifier))
                .filter(exists);
            ...
            const attachmentsWithData = await Promise.all((this.get('attachments') || []).map(loadAttachmentData));
        ...
        removeOutgoingErrors(incomingIdentifier) {
            ...
            const errors = _.partition(this.get('errors'), e => window.ConversationController.getConversationId(
                // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
                e.identifier || e.number) === incomingConversationId &&
                (e.name === 'MessageError' ||
                    e.name === 'OutgoingMessageError' ||
                    e.name === 'SendMessageNetworkError' ||
                    e.name === 'SignedPreKeyRotationError' ||
                ...
                .catch(result => {
                    ...
                    if (isError(result)) {
                        ...
                        if (result.successfulIdentifiers.length > 0) {
                            ...
                            const unregisteredUserErrors = _.filter(result.errors, error => error.name === 'UnregisteredUserError');
                            ...
                            const filteredErrors = _.reject(result.errors, error => error.name === 'UnregisteredUserError');
                        ...
                        promises = promises.concat(
                            ...
                            _.map(result.errors, error => {
                                if (error.name === 'OutgoingIdentityKeyError') {
                                    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
                                    const c = window.ConversationController.get(error.identifier || error.number);
                                    promises.push(c.getProfiles());
                                }
                            }));
                    }
        ...
        async sendSyncMessage() {
            const ourNumber = window.textsecure.storage.user.getNumber();
            const ourUuid = window.textsecure.storage.user.getUuid();
            const { wrap, sendOptions } = window.ConversationController.prepareForSend(ourUuid || ourNumber, {
                syncMessage: true,
            });
            ...
            const next = async () => {
                ...
                // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
                const conv = this.getConversation();
                return wrap(window.textsecure.messaging.sendSyncMessage(dataMessage, this.get('sent_at'), conv.get('e164'), conv.get('uuid'), this.get('expirationStartTimestamp'), this.get('sent_to'),
                    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
                    this.get('unidentifiedDeliveries'), isUpdate, sendOptions)).then(async (result) => {
                        this.set({
                            synced: true,
                            dataMessage: null,
                        });
        ...
        async queueAttachmentDownloads() {
            ...
            let count = 0;
            let bodyPending;
            window.log.info(`Queueing ${attachmentsToQueue.length} attachment downloads for message ${this.idForLogging()}`);
            const [longMessageAttachments, normalAttachments] = _.partition(attachmentsToQueue, attachment => attachment.contentType ===
                window.Whisper.Message.LONG_MESSAGE_CONTENT_TYPE);
            if (longMessageAttachments.length > 1) {
                window.log.error(`Received more than one long message attachment in message ${this.idForLogging()}`);
            }
            window.log.info(`Queueing ${longMessageAttachments.length} long message attachment downloads for message ${this.idForLogging()}`);
            if (longMessageAttachments.length > 0) {
                count += 1;
                bodyPending = true;
                await window.Signal.AttachmentDownloads.addJob(longMessageAttachments[0], {
                    messageId,
                    type: 'long-message',
                    index: 0,
                });
            ...
            if (sticker) {
                ...
                if (status && (status === 'downloaded' || status === 'installed')) {
                    try {
                        const copiedSticker = await copyStickerToAttachments(packId, stickerId);
                        data = Object.assign(Object.assign({}, copiedSticker), { contentType: 'image/webp' });
                    }
                    catch (error) {
                        window.log.error(`Problem copying sticker (${packId}, ${stickerId}) to attachments:`, error && error.stack ? error.stack : error);
                    }
                }
                if (!data) {
        ...
        async copyFromQuotedMessage(message) {
            ...
            try {
                if (queryMessage.get('schemaVersion') <
                ...
                window.log.error('Problem upgrading message quoted message from database', Errors.toLogFormat(error));
                return message;
            }
            ...
            if (queryAttachments.length > 0) {
                ...
                if (thumbnail && thumbnail.path) {
                    firstAttachment.thumbnail = Object.assign(Object.assign({}, thumbnail), { copied: true });
                }
            }
            ...
            if (queryPreview.length > 0) {
                ...
                if (image && image.path) {
                    firstAttachment.thumbnail = Object.assign(Object.assign({}, image), { copied: true });
                }
            }
            const sticker = queryMessage.get('sticker');
            if (sticker && sticker.data && sticker.data.path) {
                firstAttachment.thumbnail = Object.assign(Object.assign({}, sticker.data), { copied: true });
            }
            return message;
        }
        handleDataMessage(initialMessage, confirm, options = {}) {
            ...
            return conversation.queueJob(async () => {
                window.log.info(`Starting handleDataMessage for message ${message.idForLogging()} in conversation ${conversation.idForLogging()}`);
                ...
                if (type === 'outgoing') {
                    if (isUpdate && existingMessage) {
                        window.log.info(`handleDataMessage: Updating message ${message.idForLogging()} with received transcript`);
                        let sentTo = [];
                        ...
                        toUpdate.set({
                            ...
                            unidentifiedDeliveries: _.union(toUpdate.get('unidentifiedDeliveries'), unidentifiedDeliveries),
                        });
                    ...
                    if (isUpdate) {
                        window.log.warn(`handleDataMessage: Received update transcript, but no existing entry for message ${message.idForLogging()}. Dropping.`);
                        confirm();
                        return;
                    }
                    if (existingMessage) {
                        window.log.warn(`handleDataMessage: Received duplicate transcript for message ${message.idForLogging()}, but it was not an update transcript. Dropping.`);
                        confirm();
                        return;
                    }
                }
                ...
                if (type === 'incoming' &&
                    ...
                    window.log.warn(`Received message destined for group ${conversation.idForLogging()}, which we or the sender are not a part of. Dropping.`);
                    confirm();
                    return;
                }
                ...
                const withQuoteReference = await this.copyFromQuotedMessage(initialMessage);
                ...
                try {
                    ...
                    const preview = incomingPreview.filter((item) => (item.image || item.title) &&
                    ...
                    message.set({
                        ...
                        preview,
                        requiredProtocolVersion: dataMessage.requiredProtocolVersion ||
                            this.INITIAL_PROTOCOL_VERSION,
                    ...
                    if (isSupported) {
                        let attributes = Object.assign({}, conversation.attributes);
                        // GroupV1
                        if (!isGroupV2 && dataMessage.group) {
                            const pendingGroupUpdate = [];
                            const memberConversations = await Promise.all(dataMessage.group.membersE164.map((e164) => window.ConversationController.getOrCreateAndWait(e164, 'private')));
                            const members = memberConversations.map(c => c.get('id'));
                            attributes = Object.assign(Object.assign({}, attributes), { type: 'group', groupId: dataMessage.group.id });
                            if (dataMessage.group.type === GROUP_TYPES.UPDATE) {
                                attributes = Object.assign(Object.assign({}, attributes), { name: dataMessage.group.name, members: _.union(members, conversation.get('members')) });
                                ...
                                if (avatarAttachment) {
                                    try {
                                        downloadedAvatar = await window.Signal.Util.downloadAttachment(avatarAttachment);
                                        if (downloadedAvatar) {
                                            const loadedAttachment = await window.Signal.Migrations.loadAttachmentData(downloadedAvatar);
                                            hash = await window.Signal.Types.Conversation.computeHash(loadedAttachment.data);
                                        }
                                    }
                                    catch (err) {
                                        window.log.info('handleDataMessage: group avatar download failed');
                                    }
                                }
                                ...
                                if (
                                    ...
                                    if (downloadedAvatar && avatarAttachment !== null) {
                                        const onDiskAttachment = await window.Signal.Types.Attachment.migrateDataToFileSystem(downloadedAvatar, {
                                            writeNewAttachmentData: window.Signal.Migrations.writeNewAttachmentData,
                                        });
                                        avatar = Object.assign(Object.assign({}, onDiskAttachment), { hash });
                                    }
                                    attributes.avatar = avatar;
                                    pendingGroupUpdate.push(['avatarUpdated', true]);
                                }
                                else {
                                    window.log.info('handleDataMessage: Group avatar hash matched; not replacing group avatar');
                                }
                                const difference = _.difference(members,
                                    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
                                ...
                                const inGroup = Boolean(sender &&
                                    (conversation.get('members') || []).includes(sender.id));
                                if (!inGroup) {
                                    const senderString = sender ? sender.idForLogging() : null;
                                    window.log.info(`Got 'left' message from someone not in group: ${senderString}. Dropping.`);
                                    return;
                                }
                                if (sender.isMe()) {
                            ...
                            if (pendingGroupUpdate.length) {
                                const groupUpdate = pendingGroupUpdate.reduce((acc, [key, value]) => {
                                    acc[key] = value;
                                    return acc;
                                }, {});
                        ...
                        if (message.isEmpty()) {
                            window.log.info(`handleDataMessage: Dropping empty message ${message.idForLogging()} in conversation ${conversation.idForLogging()}`);
                            confirm();
                            return;
                        }
                        if (type === 'outgoing') {
                            const receipts = window.Whisper.DeliveryReceipts.forMessage(conversation, message);
                            receipts.forEach(receipt => message.set({
                            ...
                            if (!message.isEndSession()) {
                                if (dataMessage.expireTimer) {
                                    if (dataMessage.expireTimer !== conversation.get('expireTimer')) {
                                        conversation.updateExpirationTimer(dataMessage.expireTimer, source,
                                            // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
                                    ...
                                    conversation.updateExpirationTimer(undefined, source,
                                        // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
                                        message.get('received_at'));
                                }
                            }
                        ...
                        if (type === 'incoming') {
                            ...
                            if (readSync) {
                                if (message.get('expireTimer') &&
                                    ...
                                    message.set('expirationStartTimestamp', Math.min(readSync.get('read_at'), Date.now()));
                                }
                            }
                        ...
                        if (type === 'outgoing') {
                            const reads = window.Whisper.ReadReceipts.forMessage(conversation, message);
                            if (reads.length) {
                        ...
                        if (dataMessage.profileKey) {
                            ...
                            if (source === window.textsecure.storage.user.getNumber() ||
                        ...
                        if (type === 'incoming' &&
                            ...
                            window.log.warn(`Received tap to view message ${message.idForLogging()} with invalid data. Erasing contents.`);
                            message.set({
        ...
        async handleReaction(reaction, shouldPersist = true) {
            ...
            if (reaction.get('remove')) {
                ...
                const newReactions = reactions.filter(re => re.emoji !== reaction.get('emoji') ||
                    re.fromId !== reaction.get('fromId'));
                this.set({ reactions: newReactions });
            }
            else {
                window.log.info('Adding reaction for message', messageId);
                const newReactions = reactions.filter(re => re.fromId !== reaction.get('fromId'));
                newReactions.push(reaction.toJSON());
                this.set({ reactions: newReactions });
                const conversation = window.ConversationController.get(this.get('conversationId'));
            ...
            window.log.info(`Done processing reaction for message ${messageId}. Went from ${count} to ${newCount} reactions.`);
            if (shouldPersist) {
                await window.Signal.Data.saveMessage(this.attributes, {
                    Message: window.Whisper.Message,
                });
    ...
    window.Whisper.Message.getLongMessageAttachment = ({ body, attachments, now, }) => {
